...
def reset_database():
    try:
        ...
        return render_result(msg="success reset database")
    except:
...
def course_list():
    ...
    if User.current_login_user:
        ...
        one_page_course_list, total_pages, total_num = Course.get_courses_by_page(page)
...
def course_details():
    ...
    if User.current_login_user:
        ...
        if course_id == -1:
            ...
            course, overall_comment = model_course.get_course_by_course_id(
                int(course_id)
            )
